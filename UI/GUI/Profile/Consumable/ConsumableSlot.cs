...
namespace CovertPath.UI {
	public class ConsumableSlot : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler, IDropHandler, IBeginDragHandler, IEndDragHandler, IDragHandler {
		public Image icon;
		public Item item;
		...
		public void OnDrop(PointerEventData eventData) {
			if (_consumableCore.items[index] == _dragHolderCore.itemFirst)
				return;
			...
			if (_dragHolderCore.itemFirst != null && _dragHolderCore.itemFirst.type != ItemType.Consumable)
				return;
			// If there is item on the slot. Store it in drop holder
			_dragHolderCore.DropDrag(item);
			// Update equipment core
			_consumableCore.items[index] = _dragHolderCore.itemFirst;
			// Update slot
			AddItem(_dragHolderCore.itemFirst);
			// Updating whole equipmentcore	
			_consumableCore.onItemChangedCallback.Invoke();
		...
		public void OnEndDrag(PointerEventData eventData) {
			...
			if (_dragHolderCore.dropped == true) {
				...
				_consumableCore.items[index] = _dragHolderCore.itemSecond;
				// Update item to slot
